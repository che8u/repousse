...
void usingMetal(unsigned int vecLength) {
  ...
  // Optionally, read back results (uncomment to validate)
  // std::memcpy(vectorC.data(), pBufferC->contents(), vectorC.size() * sizeof(float));
  // validateCalc(vectorC, ...); // Provide expected results for validation

  // Cleanup
  // pCommandEncoder->release();
